...
class UserService
{
    ...
    {
        // $annonce_builder =User::query()->orderBy('id', 'asc') ;// Order by ;
        // if(!empty($relation)){
        ...
        ->get(env('MICRO_SERVICE_AUTH_URL') .'/api/all-users', $params);

        if ($response->successful()) {
            $data = $response->json()['data'];

            // Si l'API supporte déjà la pagination Laravel-like
            // if ($perPage && isset($data['data'], $data['total'])) {
            ...
            // Sinon, retourne juste une collection
            return collect($data);
        }
